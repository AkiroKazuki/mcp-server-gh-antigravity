...
export class FailureAnalyzer {
  ...
  async analyze(
    ...
    const { category, confidence, factors } = this.categorize(
      promptContent,
      outputContent,
      errors
    );
    ...
    const contextAdditions = this.generateContextAdditions(
      category,
      promptContent,
      outputContent,
      errors
  ...
  async suggestSkillUpdate(
    ...
    let rationale = '';
    let priority: SkillSuggestion['priority'] = 'medium';

    switch (analysis.category) {
      case 'incomplete_specification':
        additions.push(
          'Add checklist: Ensure all function parameters are explicitly typed',
          'Add rule: Every prompt must include expected input/output examples',
          'Add rule: Specify error handling requirements explicitly'
        );
        removals.push(
          'Remove any vague instructions like "handle appropriately"'
        );
        rationale =
      ...
      case 'missing_context':
        additions.push(
          'Add rule: Always include import paths for referenced modules',
          'Add rule: Provide type definitions for all external dependencies',
          'Add rule: Include relevant file structure context'
        );
        removals.push(
          'Remove assumptions about available imports or global state'
        );
        rationale =
      ...
      case 'wrong_pattern':
        additions.push(
          'Add section: Preferred patterns and architectures',
          'Add rule: Reference latest API versions and deprecation notices',
          'Add anti-pattern examples to avoid'
        );
        removals.push(
          'Remove references to deprecated APIs or outdated patterns'
        );
        rationale =
      ...
      case 'security_gap':
        additions.push(
          'Add mandatory security checklist before code acceptance',
          'Add rule: All user input must be validated and sanitized',
          'Add rule: No hardcoded credentials or secrets',
          'Add rule: Use parameterized queries for all database operations'
        );
        removals.push(
          'Remove any examples that demonstrate insecure practices'
        );
        rationale =
      ...
      case 'quality_issue':
        additions.push(
          'Add rule: All public functions must have type annotations',
          'Add rule: Error handling must be explicit (no empty catch blocks)',
          'Add rule: Include JSDoc/docstrings for public API'
        );
        removals.push(
          'Remove tolerance for "any" types or missing documentation'
        );
        rationale =
      ...
      default:
        additions.push(
          'Add more detailed failure logging for unclassified failures',
          'Add rule: Include validation criteria in every prompt'
        );
        rationale =
  ...
  private categorize(
    ...
    let bestCategory: FailureAnalysis['category'] = 'unknown';
    let bestScore = 0;
    let totalChecked = 0;
    let totalMatched = 0;
    ...
    for (const fp of this.failurePatterns) {
      ...
      for (const indicator of fp.indicators) {
        ...
        if (indicator.test(combinedText)) {
          ...
          totalMatched++;
          factors.push(
            `[${fp.category}] Matched: ${indicator.source.slice(0, 60)}`
          );
        }
      }
