...
export class ContextGatherer {
    ...
    async resolveImport(importRef, baseDir) {
    ...
    async findBaseClasses(filePath) {
        let content;
        try {
            content = await fs.readFile(filePath, 'utf-8');
        }
    ...
    async getRecentRelatedChanges(filePath) {
        const dirName = path.dirname(filePath);
        const maxChars = 2000;
        try {
            const { stdout } = await execFileAsync('git', ['diff', 'HEAD~5', '--', dirName], {
