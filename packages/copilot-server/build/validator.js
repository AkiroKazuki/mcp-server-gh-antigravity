...
export class Validator {
    ...
    validate(content, requirements) {
        ...
        const criticalSecurityCount = issues.filter((i) => i.type === 'security' && i.severity === 'critical').length;
        const warningSecurityCount = issues.filter((i) => i.type === 'security' && i.severity === 'warning').length;
        const qualityWarningCount = issues.filter((i) => i.type === 'quality' && i.severity === 'warning').length;
        const qualityInfoCount = issues.filter((i) => i.type === 'quality' && i.severity === 'info').length;
        const securityScore = Math.max(0, 100 - criticalSecurityCount * 25 - warningSecurityCount * 10);
        const qualityScore = Math.max(0, 100 - qualityWarningCount * 10 - qualityInfoCount * 5);
