#!/usr/bin/env bash
# Smoke test for analytics-server v2.0
# Sends MCP initialize + tools/list and verifies 13 tools are returned.
...
mkdir -p "${PROJECT_ROOT}/.memory/snapshots"
mkdir -p "${PROJECT_ROOT}/.memory/config"

cleanup() { rm -rf "$PROJECT_ROOT"; }
trap cleanup EXIT

echo "=== Analytics Server Smoke Test ==="
...
if [ "$TOOL_COUNT" -ge 13 ]; then
  echo "PASS: analytics-server returned >= 13 tools"
  exit 0
else
  echo "FAIL: expected >= 13 tools, got ${TOOL_COUNT}"
