...
class AnalyticsServer {
  ...
  constructor() {
    this.server = new Server(
      { name: "antigravity-analytics", version: "1.0.0" },
...
Token budget awareness: User pays per token for Antigravity outputs. Be concise.`,
      ...
      tools: [
        {
          name: "log_cost",
          description:
            "Log token usage and cost for an AI interaction. Antigravity costs $0.015/1K tokens, Copilot is free.",
          inputSchema: {
        ...
        },
        {
          ...
          description:
            "Get aggregated cost summary for a time period, broken down by agent.",
          inputSchema: {
            type: "object" as const,
            properties: {
              period: {
        ...
        },
        {
          ...
          description:
            "Get Copilot interaction performance metrics (average score, success rate).",
          inputSchema: {
            type: "object" as const,
            properties: {
              group_by: {
        ...
        },
        {
          name: "get_insights",
          description:
            "Analyze cost and performance data to identify optimization opportunities.",
          inputSchema: {
            type: "object" as const,
            properties: {},
          },
        },
        {
          name: "check_budget",
          description:
            "Check if budget allows an operation. Returns error if budget would be exceeded.",
          inputSchema: {
            type: "object" as const,
            properties: {
              estimated_tokens: {
  ...
  private async handleLogCost(args: any) {
    ...
    const costPerToken =
      agent === "antigravity" ? config.costs.antigravity_input : config.costs.copilot;
  ...
  private async handleCostSummary(args: any) {
  ...
  private async handlePerformance(args: any) {
    const groupBy: string = args?.group_by || "overall";
    ...
    if (groupBy === "overall") {
      ...
      const successRate =
        entries.filter((e) => e.score >= 4).length / entries.length;
    ...
    if (groupBy === "template") {
      ...
      const templateStats = Object.fromEntries(
        Object.entries(groups).map(([template, data]) => {
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { group_by: "template", templates: templateStats },
              null,
              2
            ),
          },
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { total_interactions: entries.length, group_by: groupBy },
            null,
            2
          ),
        },
  ...
  private async handleInsights() {
    ...
    try {
      ...
      if (totalWeekCost === 0) {
        insights.push(
          "No cost data this week. Start logging costs with log_cost to track spending."
        );
      } else {
        const config = this.budget.getConfig();
        ...
        if (weeklyUsage > 0.8) {
          insights.push(
            `HIGH SPEND: Weekly spending at ${(weeklyUsage * 100).toFixed(0)}% of limit. Consider using Copilot more for code generation.`
          );
        } else if (weeklyUsage < 0.2) {
          insights.push(
            `LOW USAGE: Only ${(weeklyUsage * 100).toFixed(0)}% of weekly budget used. You have capacity for more Antigravity-driven tasks.`
          );
        }
        ...
        if (total > 5 && agTotal / total > 0.7) {
          insights.push(
            `Antigravity is doing ${((agTotal / total) * 100).toFixed(0)}% of work. Delegate more code generation to Copilot to save tokens.`
          );
        }
      }
    } catch {
    ...
    try {
      ...
      if (entries.length >= 5) {
        const avgScore =
          entries.reduce((sum: number, e: any) => sum + e.score, 0) / entries.length;
        const successRate = entries.filter((e: any) => e.score >= 4).length / entries.length;

        if (avgScore < 3) {
          insights.push(
            `LOW QUALITY: Average Copilot score is ${avgScore.toFixed(1)}/5. Review and update .skills/copilot_mastery.md with better prompting techniques.`
          );
        }

        if (successRate < 0.7) {
          insights.push(
            `LOW SUCCESS RATE: Only ${(successRate * 100).toFixed(0)}% of Copilot outputs are acceptable. Consider adding more context to prompts or updating templates.`
          );
        }

        // Check for common failed issues
        const failedEntries = entries.filter((e: any) => e.score < 4 && e.issues);
        if (failedEntries.length > 0) {
          ...
          if (issueWords.includes("type") || issueWords.includes("typing")) {
            insights.push(
              "RECURRING ISSUE: Type-related failures detected. Add type hint requirements to prompt templates."
            );
          }
          if (issueWords.includes("edge case") || issueWords.includes("error")) {
            insights.push(
              "RECURRING ISSUE: Edge case handling failures. Add explicit edge case lists to prompts."
            );
          }
        }
        ...
        const recentAvg =
          recent.reduce((sum: number, e: any) => sum + e.score, 0) / recent.length;
        if (recentAvg < avgScore - 0.5) {
          insights.push(
            `DECLINING TREND: Recent scores (${recentAvg.toFixed(1)}) are lower than average (${avgScore.toFixed(1)}). Check if prompts need updating.`
          );
        } else if (recentAvg > avgScore + 0.5) {
          insights.push(
            `IMPROVING TREND: Recent scores (${recentAvg.toFixed(1)}) are higher than average (${avgScore.toFixed(1)}). Current approach is working well.`
          );
        }
      } else {
        insights.push(
          `Only ${entries.length} scored interactions so far. Need at least 5 for meaningful insights.`
        );
      }
    } catch {
      insights.push(
        "No performance data yet. Score Copilot interactions with copilot_score to track quality."
      );
    }
