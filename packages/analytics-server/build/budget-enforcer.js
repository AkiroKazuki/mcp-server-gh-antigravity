...
export class BudgetEnforcer {
    ...
    async checkBudget(estimatedTokens, agent) {
        ...
        if (projectedTotal > this.config.daily_limit_usd) {
            return {
                ...
                daily_limit: this.config.daily_limit_usd,
                warning: `BUDGET EXCEEDED: Today's spend $${todaySpend.toFixed(2)} + ` +
    ...
    async readCostLog() {
        const logFile = path.join(this.projectRoot, ".memory", "snapshots", "costs.jsonl");
        try {
