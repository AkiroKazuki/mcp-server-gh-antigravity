...
export class PerformanceProfiler {
    ...
    logOperation(server, operation, durationMs, success, metadata) {
        const stmt = this.db.prepare(`INSERT INTO performance_logs (id, server, operation, duration_ms, success, metadata)
       VALUES (?, ?, ?, ?, ?, ?)`);
        stmt.run(randomUUID(), server, operation, durationMs, success ? 1 : 0, metadata ? JSON.stringify(metadata) : null);
    }
    getProfile(timeWindow = 'day') {
        const windowMap = {
            hour: "-1 hour",
            day: "-1 day",
            week: "-7 days",
        };
        const rows = this.db.prepare(`SELECT operation, duration_ms FROM performance_logs
       WHERE timestamp >= datetime('now', ?)
       ORDER BY operation, duration_ms`).all(windowMap[timeWindow]);
        if (rows.length === 0) {
            return {
    ...
    getBottlenecks(thresholdMs = 1000) {
        const rows = this.db.prepare(`SELECT operation, AVG(duration_ms) as avg_ms, COUNT(*) as cnt
       FROM performance_logs
       WHERE timestamp >= datetime('now', '-7 days')
       GROUP BY operation
       HAVING avg_ms > ?
       ORDER BY avg_ms DESC`).all(thresholdMs);
        return rows.map((row) => {
            const impact = row.avg_ms > thresholdMs * 3 ? 'high' :
                row.avg_ms > thresholdMs * 1.5 ? 'medium' : 'low';
            const suggestions = [];
            if (row.avg_ms > 5000)
                suggestions.push('Consider adding caching for this operation');
            if (row.cnt > 100)
                suggestions.push('High call frequency — consider batching');
            if (impact === 'high')
                suggestions.push('Investigate root cause — operation significantly exceeds threshold');
            if (suggestions.length === 0)
                suggestions.push('Monitor — currently above threshold but manageable');
            return {
    ...
    getRecentLogs(limit = 20) {
        const rows = this.db.prepare(`SELECT id, timestamp, server, operation, duration_ms, success
       FROM performance_logs
