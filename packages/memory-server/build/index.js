...
class MemoryServer {
    ...
    temporal;
    constructor() {
        this.server = new Server({ name: "antigravity-memory", version: "2.0.0" }, { capabilities: { tools: {} } });
    ...
    async handleSearch(args) {
        ...
        const minConfidence = args.min_confidence;
    ...
    async handleUpdate(args) {
        ...
        const relPath = FILE_MAP[fileKey];
        if (!relPath)
            return respondError(`Unknown file: ${fileKey}`);
        const fullPath = path.join(MEMORY_PATH, relPath);
        return await this.lockManager.withLock(fullPath, async () => {
            ...
            if (operation === "append") {
                ...
                if (!section)
                    return respondError("section parameter required for update_section operation");
    ...
    async handleLogDecision(args) {
        ...
        return await this.lockManager.withLock(fullPath, async () => {
            ...
            const tEntry = this.temporal.createEntry(`Decision: ${args.title} - ${args.what}`, "decisions/ACTIVE.md", "decision", `${date}: ${args.title}`, ["decision"]);
    ...
    async handleLogLesson(args) {
        ...
        if (!relPath)
            return respondError(`Unknown lesson type: ${args.type}`);
        const fullPath = path.join(MEMORY_PATH, relPath);
        ...
        return await this.lockManager.withLock(fullPath, async () => {
            ...
            const commitHash = await this.git.getLatestCommitHash();
            const tEntry = this.temporal.createEntry(`${args.type}: ${args.category} - ${args.title}`, relPath, "lesson", `${args.category}: ${args.title}`, [args.type, args.category]);
    ...
    // New v2 handlers
    // =========================================================================
    async handleValidateMemory(args) {
    ...
    async handleDetectContradictions(args) {
        ...
        for (let i = 0; i < entries.length; i++) {
            for (let j = i + 1; j < entries.length; j++) {
                checkedPairs++;
                const similarity = await this.semantic.pairwiseSimilarity(entries[i].content, entries[j].content);
        ...
        contradictions.sort((a, b) => Math.abs(a.entry1.confidence - a.entry2.confidence) -
    ...
    async handleSuggestPruning(args) {
        const confidenceThreshold = args.confidence_threshold;
    ...
    async handleApplyPruning(args) {
        ...
        this.temporal.logOperation("apply_pruning", undefined, commitHash ?? undefined, undefined, `Archived ${result.archived} entries: ${entryIds.join(", ")}`);
        return respond({
            status: "success",
        ...
        for (const op of operations) {
            if (op.commit_hash) {
                try {
                    ...
                    const { stdout } = await execFileAsync("git", ["rev-parse", `${op.commit_hash}~1`], { cwd: MEMORY_PATH });
